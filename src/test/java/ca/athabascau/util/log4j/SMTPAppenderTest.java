...
public class SMTPAppenderTest extends TestCase
{
    ...
    private Store store;
    private Folder meFolder;
    private Folder youFolder;
    ...
    protected void setUp() throws Exception
    {
        ...
        meFolder = store.getFolder("INBOX");
        meFolder.open(Folder.READ_WRITE);
        session = Session.getInstance(new Properties());
        session.setDebug(false);
        store = session.getStore("pop3");
        store.connect("example.com", "you", "anything");
        youFolder = store.getFolder("INBOX");
        youFolder.open(Folder.READ_WRITE);
    }

    /**
     * Tests a basic error that doesn't match.  All unmatched errors should
     * be emailed out to me@example.com
     *
    ...
    /**
     * Tests that flood protection is enabled, and is functioning.
     *
     * @throws MessagingException
     * @throws IOException
     */
    public void testFloodProtection() throws MessagingException, IOException
    {
        logger.error("ErrorCommand logging: student id - null\n" +
            "ca.montage.banner.exception.BannerException: an unknown error occurred\n" +
            "\tat ca.montage.banner.web.DispatcherServlet.doGet(DispatcherServlet.java:217)\n" +
    ...
    public void testAlternateToAddress() throws IOException, MessagingException
    {
        ...
        Assert.assertFalse("flood protection message found, that shouldn't " +
            ...
            youFolder));
        final Message[] messages = youFolder.getMessages();
    }
     ...
     * @throws IOException
     */
    public boolean checkMail(final String bodyPattern, Folder folder)
